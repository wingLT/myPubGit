    ...
    <div class="content">
      <div class="cont-lf">
        ...
        <div class="track-cont">
          ...
          <div id="realtimeTrack" style="height: 867px;" class="realtime-track">
        ...
        <div class="contlr-top">
          ...
          <div class="percentage-complete">
            <div class="statis-cont">
              ...
              <div id="finshRateGrahp" class="finsh-rate"></div>
              <div class="finshrate-info">
                <div class="floor" style="margin-top: 25px;">
                  <div style="border-right: 1px solid #313132;">
                    ...
                    <span class="info-price" style="color: #a4ed8d">{{ checkComplete.done }}</span>
                  </div>
                  <div>
                    <span class="info-name">处理中</span>
                    <span class="info-price" style="color: #187dff">{{ checkComplete.solving }}</span>
                  </div>
                </div>
                <div class="floor" style="margin-top: 40px;">
                  <div style="border-right: 1px solid #313132;">
                    <span class="info-name">已超期</span>
                    <span class="info-price" style="color: #f84a46">{{ checkComplete.overtime }}</span>
                  </div>
                  <div>
                    <span class="info-name">待处理</span>
                    <span class="info-price" style="color: #ffa628">{{ checkComplete.pending }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="personpoll-dispose">
            ...
            <div id="personpollCont" class="personopll-cont">
              <div id="personpollOne">
                <ul>
                  <li v-for="(item,index) in checkTaskData" :key="index">
                    <div class="personinfo-cont">
                      <div class="info-lf">
                        <!-- <div class="infolf-icon"> -->
                          <img class="infolf-icon" :src=item.img alt="">
                          <!-- </div> -->
                      </div>
                      <div class="info-lr">
                        <p>
...
export default {
  ...
  mounted() {
    const self = this;
    self.getCheckCompleteInfo();
    self.getPersonCheckTaskInfo();
    creatMap()
    // drawSecurityLatePassage(self)
  },
  methods: {
    /*
获取巡检完成率数据信息
    ...
    async getCheckCompleteInfo() {
      ...
      let data = await checkGuard.getCheckCompleteInfo(id).then((result) => {
        if (result.status === 200) {
          ...
          const complete = result.data.done / result.data.total;
      ...
      let data = await checkGuard.getPersonCheckTaskInfo(parse).then((result) => {
        if (result.status === 200) {
          ...
          self.$nextTick(function(){
            comm.scrollTop("personpollCont", "personpollOne", "personpollTwo", 30);
          })
        } else {
          comMessageTips(self, result);
        }
...
function creatMap() {
  ...
  map = new fengmap.FMMap({
    //渲染dom
    container: document.getElementById('realtimeTrack'),
    //地图数据位置
  ...
  map.on('loadComplete', function () {
    ...
    addLocationMarker({x: 11572500.200286161, y: 3578150.7351547135});
    addLocationMarker({x: 17550000.562100000, y: 1860000.8904000000});
    //展示按模拟轨迹移动定位按钮
//      var points = points
  });
...
完成率 统计
*/
// function drawUserTendency(self, xAxisData) {
function drawSecurityLatePassage(self) {
