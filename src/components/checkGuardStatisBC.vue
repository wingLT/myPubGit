...
function creatMap(self,dotArr) {
    ...
    map = new fengmap.FMMap({
      //渲染dom
      container: document.getElementById('BCrealtimeTrack'),
      //地图数据位置
...
获取巡检完成率数据信息
*/
function getCheckCompleteInfo(self) {
  $.ajax({
    type: "GET",
    url: comm.checkGuardUrl + "/completion?id=" + self.id,
...
获取个人巡检任务处理情况数据信息
*/
function getPersonCheckTaskInfo(self) {
  $.ajax({
    type: "GET",
    url: comm.checkGuardUrl + "/task?id=" + self.id + "&porch=" + self.porch,
    success: function(result) {
      if (result.status === 200) {
        self.checkTaskData = result.data;
        self.$nextTick(function() {
...
获取地图数据信息
*/
function getTrackMapInfo(self) {
  $.ajax({
    type: "GET",
    url: comm.checkGuardUrl + "/trajectory?id=" + self.id,
    success: function(result) {
      if (result.status === 200) {
        let trackData = result.data;
        let dotArr = [];
